 ...
 */
const Node Compose::root[] = { {{2, 0}, nullptr}, 
	{{L3, KEY_TAB}, l3Tab}
};
	
const Node Compose::l3Tab[] = { {{4, 0}, nullptr}, 
	{{L1, KEY_G}, l3TabG},
	{{L3, KEY_Q}, l3TabL3Q},
	{{L3, KEY_TAB}, composeSymbols}
};

const Node Compose::l3TabG[] = { {{L1, KEY_SPACE}, composeSymbols + 1} };

const Node Compose::l3TabL3Q[] = { {{L1, KEY_G}, composeSymbols + 1} };
...
uint16_t Compose::transition(uint8_t layer, uint8_t key) {
	...
	uint16_t len = (currentNode->ptr.value == nullptr) ? cur->x.len : 1; 

	boolean found = false;
	
	//stupid linear search, could be optimized
	for (uint16_t i = 0; i < len; i++) {
		if(cur->ptr.value != nullptr && //discard length element
			cur->x.layeredKey.layer == layer && 
			cur->x.layeredKey.keycode == key) { //found
